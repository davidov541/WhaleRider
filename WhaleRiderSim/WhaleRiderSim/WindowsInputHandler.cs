...
namespace WhaleRiderSim
{
    public class WindowsInputHandler : InputHandler
    {
        ...
        public override Vector2? GetLeftClick()
        {
            ...
            if (this._previousmousestate == null || this._currentmousestate == null)
            {
                return null;
            }
            if (this._previousmousestate.LeftButton == ButtonState.Pressed &&
                this._currentmousestate.LeftButton == ButtonState.Released)
            {
                return new Vector2((float)this._currentmousestate.X / (float)this._width, (float)this._currentmousestate.Y / (float)this._height);
            }
            return null;
        }
        ...
        {
            if (this._lastcharpressed != '\0' && this._currentkeyboardstate.GetPressedKeys().Length == 1)
            {
                char c = this._lastcharpressed;
                this._lastcharpressed = '\0';
                return c;
            }
            else if (this._lastcharpressed == '\0' && this._currentkeyboardstate.GetPressedKeys().Length > 1)
            {
